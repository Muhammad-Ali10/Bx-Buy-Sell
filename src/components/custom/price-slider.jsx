...
import { useSliderWithInput } from "@/components/custom/hooks/usePriceSlider";
...
function PriceSlider2({value, onValueChange, min, max, step}) {
  const id = useId();
...
//   const maxValue = Math.max(...items.map((item) => item.price));
  ...
  const {
    ...
    handleInputChange,
    handleSliderChange,
  } = useSliderWithInput({ minValue, maxValue, initialValue: value });
  ...
  const itemCounts = Array(tick_count)
    .fill(0)
    .map((_, tick) => {
      const rangeMin = value[0] + tick * priceStep;
      const rangeMax =  value[1] + (tick + 1) * priceStep;
      ...
      <div>
        <div className="flex h-12 w-full items-end px-3" aria-hidden="true">
          {itemCounts.map((count, i) => (
            <div
              key={i}
              className="flex flex-1 my-2 justify-center"
              style={{
            ...
            >
              <span
                ...
                className="  min-w-[4.76px] border-[2px] border-[#242425]  bg-[#C5FD1F]"
              ></span>
            </div>
          ))}
        </div>
