...
import AddManagementQuestion from "@/components/admin/add-management-question"
import { useGetAdminQuestion, useDeleteAdminQuestion } from "@/hooks/adminQuestions"

const Management = () => {
    const { data, isPending, error } = useGetAdminQuestion("MANAGEMENT")
    ...
    const handleDelete = (id) => {
        ...
        DeleteAdminQuestions(id, {
            onSuccess: () => {
                console.log("Statistic Question deleted successfully")
            },
            onError: (err) => {
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <Dialog>
                                        ...
                                        <DialogContent className="p-0 border-0 rounded-2xl shadow-2xl w-full flex flex-col !max-w-[720px] h-full">
                                            <DialogHeader className="px-6 pt-6 pb-2">
                                                <DialogTitle className="text-2xl font-medium font-lufga text-black">
                                                    Update Statistic Question
                                                </DialogTitle>
                                            </DialogHeader>
                                            <div className="px-6 pb-6 h-full">
                                                <AddManagementQuestion mode="edit" question={item.question} id={item.id} />
                                            </div>
                                        </DialogContent>
                                    </Dialog>
