import Chat from "./chat.svg";
import Gear from "./gear.svg";
import Menu from "./menu.svg";
import Signout from "./signout.svg";
import VerifyUser from "./verify-user.svg";
import UserRounded from "./user-rounded.svg";
import Heart from "./heart.svg";
import Calender from "./calender.svg";
import Arrow from "./arrow.svg";
import Verified from "./verified.svg";
import Star from "./star.svg";
import Chat1 from "./chat1.svg";
import Block from "./block.svg"
import Delete from "./delete.svg"
import Edit from "./edit.svg"
import ArrowRight from "./arrow-right.svg"
import View from "./view.svg"
import Edit1 from "./edit1.svg"
import Pro from "./pro.svg"
import Alart from "./alart.svg"
import Search from "./search.svg"
import Location from "./location.svg"
import Filter from "./filter.svg"
import Archived from "./Archived.svg"
import Online from "./online.svg"
import Pin from "./pin.svg"
import Camera from "./camera.svg"
import Menu2 from "./menu2.svg"
import File from "./fileupload.svg"
import Send from "./send.svg"
import Docs from "./docs.svg"
import ChatLabel from "./chatlabel.svg"
import Report from "./Report.svg"
import Offer from "./Offer.svg"
import AppIcon from "./AppIcon.svg"
import Request from "./request.svg"
import FaceScan from "./FaceScanSquare.svg"
import Url from "./url.svg"
import Alerts from "./Alerts.svg"
import Searchlist from "./Searchlist.svg"
import WordAlerts from "./wordalerts.svg"
import Count from "./count.svg"
import Upload from "./upload.svg"
import Category from "./Category.svg"
import BrandInformation from "./BrandInformation.svg"
import Tools from "./Tools.svg"
import Financials from "./Financials.svg"
import AdditionalInfos from "./AdditionalInfos.svg"
import Accounts from "./Accounts.svg"
import AdInformations from "./AdInformations.svg"
import Handover from "./Handover.svg"
import Packages from "./Packages.svg"   
import Statistics from "./Statistics.svg"
import Product from "./Products.svg"
import Management from "./Management.svg"
import CricleCheck from "./criclecheck.svg"
import Eye from "./eye.svg"
import Crown from "./crown.svg"
import Money from "./money.svg"
import UserLocation from "./userlocation.svg"
import UserPhone from "./userphone.svg"
import UserEmail from "./useremail.svg" 
import StepRight from "./stepright.svg"
import X from "./x.svg"
import Amazon from "./amazon.svg"
import Shopify from "./shopify.svg"
import WooCommerce from "./woo.svg"
import Ebay from "./ebay.svg"
import Etsy from "./etsy.svg"
import Big_Ecommerce from "./big-ecommerce.svg"
import Magento from "./Magento.svg"
import PrestaShop from "./PrestaShop.svg" 
import Logo from "./logo.svg"
import Settings from "./settings.svg"
import Log_Out from "./log_out.svg"




export const ChatSVG = Chat;
export const GearSVG = Gear;
export const MenuSVG = Menu;
export const SignOutSVG = Signout;
export const VerifyUserSVG = VerifyUser;
export const UserRoundedSVG = UserRounded;
export const HeartSVG = Heart;
export const CalenderSVG = Calender;
export const ArrowSVG = Arrow;
export const VerifiedSVG = Verified;
export const StarSVG = Star;
export const Chat1SVG = Chat1;
export const BlockSVG = Block;
export const DeleteSVG = Delete;
export const EditSVG = Edit;
export const ArrowRightSVG = ArrowRight;
export const ViewSVG = View;
export const Edit1SVG = Edit1;
export const ProSVG = Pro;
export const AlartSVG = Alart;
export const SearchSVG = Search;
export const LocationSVG = Location;
export const FilterSVG = Filter;
export const ArchivedSVG = Archived;
export const OnlineSVG = Online;
export const PinSVG = Pin;
export const CameraSVG = Camera;
export const Menu2SVG = Menu2;
export const FileSVG = File;
export const SendSVG = Send;
export const DocsSVG = Docs;
export const ChatLabelSVG = ChatLabel;
export const ReportSVG = Report;
export const OfferSVG = Offer;
export const AppIconSVG = AppIcon;
export const RequestSVG = Request;
export const FaceScanSVG = FaceScan;
export const UrlSVG = Url;
export const AlertsSVG = Alerts;
export const SearchlistSVG = Searchlist;
export const WordAlertsSVG = WordAlerts;
export const CountSVG = Count;
export const UploadSVG = Upload;
export const CategorySVG = Category;
export const BrandInformationSVG = BrandInformation;
export const ToolsSVG = Tools;
export const FinancialsSVG = Financials;
export const AdditionalInfosSVG = AdditionalInfos;
export const AccountsSVG = Accounts;
export const AdInformationsSVG = AdInformations;
export const HandoverSVG = Handover;
export const PackagesSVG = Packages;
export const StatisticsSVG = Statistics;
export const ProductSVG = Product;
export const ManagementSVG = Management;
export const CricleCheckSVG = CricleCheck
export const EyeSVG = Eye
export const CrownSVG = Crown
export const MoneySVG = Money
export const UserLocationSVG = UserLocation
export const UserPhoneSVG = UserPhone
export const UserEmailSVG = UserEmail   
export const StepRightSVG = StepRight
export const XSVG = X
export const AmazonSVG = Amazon
export const ShopifySVG = Shopify
export const WooCommerceSVG = WooCommerce
export const EbaySVG = Ebay
export const EtsySVG = Etsy
export const Big_EcommerceSVG = Big_Ecommerce
export const MagentoSVG = Magento
export const PrestaShopSVG = PrestaShop 
export const LogoSVG = Logo
export const SettingsSVG = Settings
export const Log_OutSVG = Log_Out











