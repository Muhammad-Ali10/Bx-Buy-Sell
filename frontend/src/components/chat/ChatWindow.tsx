...
export const ChatWindow = ({ conversationId, currentUserId, userId, sellerId, listingId, refreshConversations, initialSearchQuery, onOpenDetails, sendAsTeam = false }: ChatWindowProps) => {
  ...
  /*
   * The video button. Calls run in the CallCenter at the root of the app now
   * (on Agora): the person called hears it on any page of the site, not only
   * with this chat open, and the call survives leaving this screen.
   */
  const handleVideoCall = () => {
    const chatIdToUse = chatRoom?.id || conversationId;
    // On the admin's view of a member's chats the thread is shown as that
    // member sees it; the team calling from there means calling that member.
    const otherUserId =
      sendAsTeam && currentUserId && currentUserId !== currentUser?.id
        ? currentUserId
        : currentUserId === userId
          ? sellerId
          : userId;
    if (!chatIdToUse || !otherUserId) {
      toast.error("A call cannot be started in this conversation");
      return;
    }
    startCall({ chatId: chatIdToUse, to: otherUserId });
  };

  // The dialog that opened the old one-to-one call, kept while calls placed
  // from tabs opened before the change can still arrive.
  const openLegacyVideoCallDialog = () => {
